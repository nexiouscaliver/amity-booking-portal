...
def send_mail4(event_name,event_venue,event_date,start_time,end_time,school_name,resource_person_name,resource_person_details,status,user_email,bid):
    ...
    Kindly check for more details inAmiEventHub.For your convenience, please click on the link below to visit AmiEventHUB
    ...
    Shortcut Links:
        ...
        User Page :  <a href='{server_link}'>Login Link</a>
    ...
    # Send the email
    try:
        msg = Message(subject=f'New Event: {event_name} Update dated {event_date}', recipients=[it_email])
        msg.body = email_body
        msg.html = html
        mail.send(msg)
        return True
