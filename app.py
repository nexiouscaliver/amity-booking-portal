            ...
            if session['username']:
                ...
                if db.reject_app(bid=int(bid),status=status):
                    ...
                    if status == "Pending":
                        ...
                        send_mail5(event_name,event_venue,event_date,start_time,end_time,school_name,status,bid,[admin_email,email])
                        output = db.check_request(username)
                        return render_template("statuspage.html",output=output,error=error)
                    else:
                        error = "Venue hall request Cancelled Successfully."
                        send_mail5(event_name,event_venue,event_date,start_time,end_time,school_name,status,bid,[it_email,admin_email,admin2_mail,email])
                        output = db.check_request(username)
                        return render_template("statuspage.html",output=output,error=error)
                else:
